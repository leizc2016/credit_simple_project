 ...
 */
@Controller
@RequestMapping(value = "/personalBorrow")
public class PersonalBorrowingController {
    ...
    public String fullCreditReport( Model model,
        ...
        return "/report/personalBorrowReport";
    }
...
//		String idCardNum = (String)searchParams.get("idCardNum");
		...
		//判断是否现金支付
		boolean cashflag = creditSearchService.checkIsCashPay(MenuConstant.personalBorrowingCode);
		if(!fromCache){
		    charge=creditSearchService.calculateCharge(searchType
		    		, MenuConstant.personalBorrowingCode, noResult, insCode);//计算出费用   
            //验证余额大于费用
...
//		creditSearchService.insertAccQueryHistory(searchType, idCardNum, productFullCreditId, insAccId);
		creditSearchService.recordFullFoot(
				...
				MenuConstant.personalBorrowingCode, userId);
		if(fromCache){
			generateTrans(
					charge, 
					idCardNumDecode, 
...
//		    insService.updateValueTransactionLog(Sessions.insCode(),charge,"使用24小时征信报告查询缓存，扣费0元.");
			...
			String description = "个人借贷报告查询："+ IdcardUtils.hideIdCardBirthInfo(idCardNumDecode);
			charge = creditSearchService.adjustCharge(personAccDetails, charge, idCardNum, searchType, Sessions.insCode(), MenuConstant.personalBorrowingCode);
