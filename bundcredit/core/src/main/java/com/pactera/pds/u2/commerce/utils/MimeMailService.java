 ...
 * MIME邮件服务类.
 * 
 * 使用由Freemarker引擎生成的的html格式邮件, 并带有附件.
 * 
 */
@Service
public class MimeMailService {
	 ...
	 * 发送MIME格式的用户修改通知邮件.
	 */
	public void sendNotificationMail(String valCode,String email) {
		...
		try {
			...
			helper.setTo(email);
			helper.setFrom("bgi@service.genomics.cn");
			helper.setSubject("用户注册通知");

			String content = generateContent(valCode);
...
//			helper.addAttachment("mailAttachment.txt", attachment);
	...
	private String generateContent(String userName) throws MessagingException {
		...
		try {
			Map context = Collections.singletonMap("valCode", userName);
